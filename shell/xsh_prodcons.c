...
shellcmd xsh_prodcons(int nargs, char *args[])
{
  ...
  if(nargs == 2) {
    if(strncmp(args[1],"-f",3) == 0) {
      ...
      // Test FUTURE_SHARED
      conpid[ind] = create(future_cons, 1024, 20, "fcons2",1,f_shared);
      ind++;
      conpid[ind] = create(future_cons, 1024, 20, "fcons3",1,f_shared);
      ind++;
      conpid[ind] = create(future_cons, 1024, 20, "fcons4",1,f_shared);
      ind++;
      conpid[ind] = create(future_cons, 1024, 20, "fcons5",1,f_shared);
      ind++;
      conpid[ind] = create(future_prod, 1024, 20, "fprod2",1,f_shared);
      ind++;	 	 

      // Test FUTURE_QUEUE
      conpid[ind] = create(future_cons, 1024, 20, "fcons6",1,f_queue);
      ind++;
      conpid[ind] = create(future_cons, 1024, 20, "fcons7",1,f_queue);
      ind++;
      conpid[ind] = create(future_cons, 1024, 20, "fcons7",1,f_queue);
      ind++;
      conpid[ind] = create(future_cons, 1024, 20, "fcons7",1,f_queue);
      ind++;
      conpid[ind] = create(future_prod, 1024, 20, "fprod3",1,f_queue);
      ind++;
      conpid[ind] = create(future_prod, 1024, 20, "fprod4",1,f_queue);
      ind++;
